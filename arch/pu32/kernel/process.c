...
int copy_thread (
	...
	if (p->flags&PF_KTHREAD) {
		...
		if (usp) // Setup usermode stack if specified.
			ppr->regs.sp = usp;
		// For CLONE_SETTLS set thread_info.tls to the tls argument passed to sys_clone.
		// The kernel entry is:
		// 	int clone (
		...
		// 		unsigned long tls);
		// The TLS argument is the 5th argument.
	...
	while (1) {
		...
		switch (faultreason) {
					...
					case 0x01: /* syscall */ {
						...
						if (pu32_in_userspace(ti)) {
							...
							case __NR_PU32_switch_to: {
								...
								static struct mm_struct *prev_userspace_mm = 0;
								if (next_mm != prev_userspace_mm) {
									prev_userspace_mm = next_mm;
			...
			case pu32AlignFaultIntr: {
				...
				if (faultreason != pu32AlignFaultIntr) {
					...
					if ((pu32_tlb_lookup(faultaddr) & permtocheck) != permtocheck) {
