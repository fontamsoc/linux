...
static inline int arch_spin_trylock (arch_spinlock_t *m) {
	...
	ret = 1;
	done_unlock:
	arch_xchg(&m->lock[0/*LOCK*/], 0); // m->lock[0/*LOCK*/] = 0;
	done:
	return ret;
}

static inline void arch_spin_lock (arch_spinlock_t *m) {
	while (arch_xchg(&m->lock[0/*LOCK*/], 1))
		/*cpu_relax()*/;
...
static inline void arch_spin_unlock (arch_spinlock_t *m) {
	arch_xchg(&m->lock[2/*CURRENT*/], (m->lock[2/*CURRENT*/]+1)); // ++m->lock[2/*CURRENT*/];
}
...
static inline void arch_read_lock (arch_rwlock_t *m) {
	...
	if (arch_spin_is_locked((arch_spinlock_t *)m)) {
		...
		while (arch_xchg(&m->lock[0/*LOCK*/], 1))
			/*cpu_relax()*/;
		arch_xchg(&m->lock[2/*CURRENT*/], (i+1)); // ++m->lock[2/*CURRENT*/];
	}
	++m->lock[3/*READERCNT*/];
	arch_xchg(&m->lock[0/*LOCK*/], 0); // m->lock[0/*LOCK*/] = 0;
}
...
static inline void arch_write_unlock (arch_rwlock_t *m) {
	arch_spin_unlock((arch_spinlock_t *)m); // Single instruction; no need to irq save/restore.
}
